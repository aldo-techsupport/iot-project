...
return new class extends Migration
{
    ...
    {
        // Update noise_calculations table - modify enum to add L5-L8 (MySQL only; SQLite has no enforced enum)
        ...
        // Update noise_raw_data table - modify enum to add L5-L8 (MySQL only)
        if (DB::getDriverName() === 'mysql') {
    ...
    {
        // Revert noise_calculations table (MySQL only)
        if (DB::getDriverName() === 'mysql') {
        ...
        // Revert noise_raw_data table (MySQL only)
        if (DB::getDriverName() === 'mysql') {
